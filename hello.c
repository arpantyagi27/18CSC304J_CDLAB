...
main = Tk()
main.title('Sagar School Management System')
main.geometry('1200x600')
main.resizable(0, 0)

# Creating the background and foreground color variables
lf_bg = 'turquoise' # bg color for the left_frame
cf_bg = 'aquamarine' # bg color for the center_frame
...
Label(main, text="SCHOOL MANAGEMENT SYSTEM", font=headlabelfont, bg='cyan').pack(side=TOP, fill=X)
...
Label(left_frame, text="Name", font=labelfont, bg=lf_bg).place(relx=0.300, rely=0.05)
Label(left_frame, text="Contact Number", font=labelfont, bg=lf_bg).place(relx=0.135, rely=0.18)
...
Label(left_frame, text="Date of Birth", font=labelfont, bg=lf_bg).place(relx=0.16, rely=0.57)
...
dob = DateEntry(left_frame, font=("Times New Roman", 12), width=15)
dob.place(x=20, rely=0.62)

Button(left_frame, text='Submit and Add Record', font=labelfont, command=add_record, width=18).place(relx=0.10, rely=0.85)
...
Label(right_frame, text='Students Records', font=headlabelfont, bg='SeaGreen3', fg='LightCyan').pack(side=TOP, fill=X)
